...
const Registration = () => {
  ...
  // Mutation
  /*  -> mutate or mutateAsync: Functions to actually trigger the registerAPI call.
`isLoading / isPending`: Boolean indicating if the mutation is currently running.
`isError`: Boolean indicating if the mutation failed.
isSuccess: Boolean indicating if the mutation succeeded.
`error`: If isError is true, this will hold the error object.
`data`: If isSuccess is true, this will hold the data returned by registerAPI (which is response?.data).

  */


  // ** useMutation: This is a React Hook provided by libraries like React Query. It's specifically designed for performing asynchronous operations that change data on the server or have side effects
  ...
  return (
    ...
    {mutation.isError && (
      <StatusMessage type="error" message={mutation?.error?.response?.data?.message} />
        ...
        {/* ! Display Success */}
    {mutation.isSuccess && (
      <StatusMessage type="success" message='Registeration success' />
    )}
